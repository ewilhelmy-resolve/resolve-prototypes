# Story 1: Database Foundation - Implementation Plan

## Overview

Create database schema for RITA Autopilot: 6 tables, RLS policies, indexes, constraints, triggers.

**Migration file:** `138_add_autopilot_tables.sql`

---

## Pre-Implementation Checklist

- [ ] Verify `update_updated_at_column()` function exists (defined in `01-init.sql`)
- [ ] Confirm middleware sets `app.current_organization_id` session var
- [ ] Review existing RLS patterns in `102_row_level_security.sql`

---

## Implementation Steps

### Step 1: Add Jira to Data Source Constants

**File:** `packages/api-server/src/constants/dataSources.ts`

```typescript
// Add to ALLOWED_DATA_SOURCE_TYPES array:
...
// Add to DEFAULT_DATA_SOURCES array:
{
...
**File:** `packages/api-server/src/database/migrations/138_add_autopilot_tables.sql`

#### 2.1 Create `clusters` Table
...
#### 2.3 Create `analytics_cluster_daily` Table
...
#### 2.4 Create `knowledge_articles` Table
...
#### 2.5 Create `ingestion_runs` Table
...
#### 2.6 Create `credential_delegation_tokens` Table
...
**File:** `packages/api-server/src/database/migrations/138_add_autopilot_tables.rollback.sql`

```sql
-- Rollback: Remove autopilot tables
-- Run this to undo migration 138
...
\dt clusters tickets analytics_cluster_daily knowledge_articles ingestion_runs credential_delegation_tokens

-- Verify RLS enabled
SELECT tablename, rowsecurity FROM pg_tables
WHERE tablename IN ('clusters', 'tickets', 'analytics_cluster_daily', 'knowledge_articles', 'ingestion_runs', 'credential_delegation_tokens');

-- Verify unique constraints
\d clusters
\d tickets
\d knowledge_articles
\d credential_delegation_tokens
...
\di idx_knowledge*
\di idx_ingestion*
\di idx_cred_delegation*
\di idx_analytics*

-- Test RLS isolation
SET LOCAL app.current_organization_id = 'some-uuid';
SELECT * FROM clusters; -- should return empty, not error
```
...
| File | Action |
|------|--------|
| `packages/api-server/src/constants/dataSources.ts` | Add `jira` to arrays |
| `packages/api-server/src/database/migrations/138_add_autopilot_tables.sql` | Create new |
| `packages/api-server/src/database/migrations/138_add_autopilot_tables.rollback.sql` | Create new |
...
- [ ] Rollback script removes all 6 tables
- [ ] RLS policies block cross-org access
- [ ] Unique constraints enforce idempotency
- [ ] `update_updated_at_column()` triggers fire on UPDATE
- [ ] Foreign key cascades work (delete org → deletes clusters → deletes tickets)
- [ ] TypeScript compiles with new `jira` type

---

## Open Items (TBD - not blocking)

- `validation_target` default value - leave nullable for now
- Analytics retention period - no action needed for schema
