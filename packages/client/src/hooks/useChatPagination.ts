...
export function useChatPagination({
	...
	const { chatMessages, setMessages, setHasMoreMessages, setLoadingMore } =
	...
	useEffect(() => {
		...
		const allMessages = data.pages.flatMap(
			(page: PaginationPage) => page.messages,
		);
		const hasMore =
			(data.pages[data.pages.length - 1] as PaginationPage)?.hasMore || false;
	...
	useEffect(() => {
		if (
			...
			scrollPositionRef.current.height > 0
		) {
			const container = scrollContainerRef.current;
			const heightDifference =
				container.scrollHeight - scrollPositionRef.current.height;
