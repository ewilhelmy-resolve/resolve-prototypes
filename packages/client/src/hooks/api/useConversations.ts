...
import { CHAT_PAGINATION, SIDEBAR_PAGINATION } from "@/constants/pagination";
...
// Fetch conversations with infinite scroll pagination (offset-based)
export function useInfiniteConversations() {
	...
	const query = useInfiniteQuery<
		...
		number
	>({
		queryKey: [...conversationKeys.lists(), "infinite"],
		queryFn: async ({ pageParam }) => {
			const response = await conversationApi.getConversations({
				limit: SIDEBAR_PAGINATION.PAGE_SIZE,
				offset: pageParam,
			});
	...
	// Sync flattened pages to Zustand store
	useEffect(() => {
		...
		queryFn: async ({ pageParam }: { pageParam: string | undefined }) => {
			...
			const response = await conversationApi.getConversationMessages(
				conversationId,
				{
					limit:
						pageParam === undefined
							? CHAT_PAGINATION.INITIAL_PAGE_SIZE
							: CHAT_PAGINATION.PAGE_SIZE,
...
export function useDeleteConversation() {
	const queryClient = useQueryClient();
	const { setConversations, currentConversationId, clearCurrentConversation } =
