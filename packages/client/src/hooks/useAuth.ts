...
export function useAuth() {
  ...
  const computedValues = useMemo(() => ({
    isTokenExpired: store.tokenExpiry ? Date.now() / 1000 > store.tokenExpiry : false,
    timeToExpiry: store.tokenExpiry ? Math.max(0, store.tokenExpiry - Date.now() / 1000) : null,
    isSessionExpired: store.sessionExpiry ? Date.now() / 1000 > store.sessionExpiry : false,
  ...
  return {
    ...
    hasValidSession: () => store.sessionReady && !computedValues.isSessionExpired,

    // User info helpers
    getUserId: () => store.user?.id,
    getUserEmail: () => store.user?.email,
    getUserName: () => store.user?.username || (store.user as any)?.preferredUsername,
    getFullName: () => {
    ...
    // Permission helpers (can be extended later)
    hasRole: (_role: string) => {
      // This would need to be implemented based on your Keycloak role structure
      return false;
    },
