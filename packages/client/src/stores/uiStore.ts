	...
	devtools(
		persist(
			(set) => ({
				// Initial state
				isSidebarOpen:
					typeof window !== "undefined" ? window.innerWidth >= 1024 : true, // Open on desktop by default
				sidebarWidth: 280,
				theme: "system",
				headerStyle: "rita",
				...
				setSounds: (enabled) => set({ enableSounds: enabled }),

				setMessageGrouping: (enabled) => set({ messageGrouping: enabled }),
