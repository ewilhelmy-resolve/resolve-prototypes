    ...
    immer((set, get) => {
      ...
      const setupEventListeners = () => {
        authManager.on('auth:success', (result) => {
          console.log('AuthStore: Received auth:success event', result);
          set((state) => {
        ...
        authManager.on('token:refreshed', ({ token, refreshToken, tokenExpiry }) => {
        ...
        authManager.on('session:ready', () => {
          console.log('AuthStore: Received session:ready event');
          set((state) => {
            state.sessionReady = true;
          });
        });
      ...
      return {
        ...
        initialize: async () => {
          ...
          try {
            ...
            set((state) => {
              state.error = error instanceof AuthError ? error : new AuthError('INIT_FAILED', 'Unknown error');
              state.loading = false;
              state.initialized = true;
            });
          }
        ...
        login: async (redirectPath) => {
          ...
          try {
            ...
            set((state) => {
              state.error = error instanceof AuthError ? error : new AuthError('LOGIN_FAILED', 'Login failed');
            });
          }
        },
        ...
        logout: async (redirectUri) => {
          ...
          try {
            await authManager.logout({ redirectUri: redirectUri || window.location.origin });
          } catch (error) {
        ...
        refreshAuthToken: async () => {
          ...
          try {
            ...
            set((state) => {
              state.error = error instanceof AuthError ? error : new AuthError('TOKEN_REFRESH_FAILED', 'Token refresh failed');
            });
            return false;
          }
        },
        ...
        retry: async () => {
          const state = get();
          console.log('AuthStore: Retrying initialization, attempt:', state.retryCount + 1);

          if (state.retryCount >= 3) {
            set((state) => {
              state.error = new AuthError('MAX_RETRIES', 'Maximum retry attempts exceeded');
            });
            return;
          }
