...
describe("conversationStore - Reasoning Message Merging", () => {
			...
			const messages: Message[] = [
				{
					...
					response_group_id: "group-1",
					metadata: {
						reasoning: { content: "Planning approach", title: "Planning" },
					},
				},
				{
					id: "msg-2",
					...
					response_group_id: "group-1",
					metadata: {
						reasoning: { content: "Analyzing data", title: "Analysis" },
					},
				},
				{
					id: "msg-3",
					...
					response_group_id: "group-1",
					metadata: {
						reasoning: { content: "Final verification", title: "Verification" },
					},
				},
			];
	...
	describe("Merge Boundary Conditions", () => {
		it("should NOT merge reasoning separated by non-reasoning text", () => {
			...
			expect(grouped.parts[1].message).toBe(
				"Interrupting text without reasoning",
			);
			expect(grouped.parts[2].metadata.reasoning.content).toBe(
				"Second reasoning",
			);
		});
	});
	...
	describe("Metadata Preservation", () => {
		...
		it("should use last message as base when it has text content", () => {
			...
			expect(part.metadata.tasks).toEqual([
				{ title: "Task", items: ["Item 1"] },
			]);
		});
	});
