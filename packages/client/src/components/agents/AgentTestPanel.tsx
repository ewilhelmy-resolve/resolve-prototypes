...
export function AgentTestPanel({
  ...
  const isReady = !!(config.name && (config.instructions || config.workflows.length > 0 || config.knowledgeSources.length > 0));
  const hasStarters = starters.length > 0;

  return (
    <div className={cn("flex flex-col bg-gradient-to-b from-slate-50 to-white rounded-xl border border-border shadow-sm", className)}>
      {/* Header */}
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex items-center gap-3 pb-4 mb-4 border-b">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground line-clamp-1">{config.description || "No description"}</p>
              </div>
            </div>
            ...
            {hasStarters ? (
              <div className="space-y-2 flex-1">
                <p className="text-xs text-muted-foreground">Conversation starters:</p>
            ...
            {(config.workflows.length > 0 || config.knowledgeSources.length > 0) && (
              <div className="pt-4 mt-4 border-t space-y-2">
                {config.workflows.length > 0 && (
                  <div className="text-xs text-muted-foreground">
                    <span className="font-medium">{config.workflows.length}</span> skill{config.workflows.length > 1 ? 's' : ''} configured
                  </div>
                )}
                {config.knowledgeSources.length > 0 && (
                  <div className="text-xs text-muted-foreground">
                    <span className="font-medium">{config.knowledgeSources.length}</span> knowledge source{config.knowledgeSources.length > 1 ? 's' : ''}
                  </div>
                )}
              </div>
            )}
