...
const DEFAULT_AGENT: Partial<AgentDefinition> = {
	...
	guardrails: [{ type: "pii_redaction", enabled: true }],
};
		...
		<div className="h-full flex flex-col">
			...
			<div className="px-4 py-3 bg-white border-b flex items-center justify-between">
				<div className="flex items-center gap-2">
					{(["setup", "build", "preview", "publish"] as BuilderPhase[]).map(
						(p, i) => (
							<button
								key={p}
								onClick={() => setPhase(p)}
		...
		<div className="relative">
			<div className="px-4 py-2 bg-slate-900 text-white flex items-center justify-between">
				<div className="flex items-center gap-3">
					...
					>
						<div
							className={`w-5 h-5 rounded ${currentConcept.color} flex items-center justify-center`}
						>
						...
						<div className="p-2">
							{CONCEPTS.map((c) => {
								...
								return (
									...
									>
										<div
											className={`w-9 h-9 rounded-lg ${c.color} flex items-center justify-center text-white`}
										>
			...
			<div className="max-w-2xl mx-auto space-y-8">
				<section className="space-y-4">
					...
					<textarea
						...
						onChange={(e) =>
							onChange({ ...agent, description: e.target.value })
						}
					/>
				</section>
						...
						{(["keyword", "intent", "scheduled", "api"] as const).map((t) => (
							<button
								...
								className={`px-4 py-2 rounded-lg border capitalize ${
									agent.trigger?.type === t
										? "bg-primary text-primary-foreground"
										: "hover:bg-muted"
								}`}
								onClick={() =>
									onChange({ ...agent, trigger: { type: t, value: "" } })
								}
							>
								{t}
							</button>
					...
					{agent.trigger && (
						<input
							...
							onChange={(e) =>
								onChange({
									...agent,
									trigger: { ...agent.trigger!, value: e.target.value },
								})
							}
						/>
					)}
					...
					{messages.length === 0 && (
						<div className="text-center text-muted-foreground py-12">
							...
							<div className="flex flex-wrap justify-center gap-2 mt-4">
								{["What can you help me with?", "Hello", "I need help"].map(
									(q) => (
										<button
											key={q}
			...
			<div className="max-w-2xl mx-auto space-y-8">
				<div>
					...
					<p className="text-muted-foreground">
						Review your agent before going live
					</p>
				</div>
				...
				<Card className="p-6 space-y-4">
					...
					<ul className="space-y-2">
						{[
							...
							{
								label: "Completion rules set",
								passed: (agent.completionRules?.length || 0) > 0,
							},
						].map((item) => (
