...
export function WizardBuilder({
	...
	const handlePreviewSend = () => {
		...
		setTimeout(() => {
			setPreviewMessages((prev) => [
				...prev,
				{ role: "agent", content: responses[tone] },
			]);
		}, 500);
		setPreviewInput("");
	};
				...
				<div className="flex-1 overflow-auto p-4">
					{activeTab === "knowledge" && (
						<KnowledgeTab
							...
							onChange={(knowledgeSources) =>
								onChange({ ...agent, knowledgeSources })
							}
						/>
					)}
					{activeTab === "persona" && (
					...
					{activeTab === "completion" && (
						<CompletionTab
							...
							onChange={(completionRules) =>
								onChange({ ...agent, completionRules })
							}
						/>
					)}
...
function KnowledgeTab({ sources, onChange }: KnowledgeTabProps) {
	const availableSources = [
		{
			id: "benefits_handbook",
			name: "Benefits Handbook 2024.pdf",
			type: "file" as const,
		},
		{
			id: "hr_confluence",
...
function CompletionTab({ rules, onChange }: CompletionTabProps) {
	...
	const toggleRule = (type: CompletionRule["type"]) => {
		...
		if (existing) {
			onChange(
				rules.map((r) => (r.type === type ? { ...r, enabled: !r.enabled } : r)),
			);
		} else {
			onChange([
				...rules,
				{
					...
					enabled: true,
					value:
						type === "max_turns" ? 10 : type === "timeout" ? 30 : undefined,
				},
			]);
		}
	};
								...
								<div className="flex-1">
									...
									{ruleType.hasValue && isEnabled && (
										<input
											...
											value={rule?.value || 10}
											onChange={(e) =>
												updateValue(ruleType.type, parseInt(e.target.value, 10))
											}
										/>
									)}
								</div>
...
function GuardrailsTab({ guardrails, onChange }: GuardrailsTabProps) {
	...
	const toggleGuardrail = (type: Guardrail["type"]) => {
		...
		if (existing) {
			onChange(
				guardrails.map((g) =>
					g.type === type ? { ...g, enabled: !g.enabled } : g,
				),
			);
		} else {
					...
					return (
						<Card key={guardType.type} className="p-4">
							<div className="flex items-start justify-between gap-4">
								...
								<Switch
									checked={isEnabled}
									onCheckedChange={() => toggleGuardrail(guardType.type)}
								/>
							</div>
						</Card>
					);
