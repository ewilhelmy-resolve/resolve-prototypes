...
export function GuidedBuilder() {
	...
	const tabs = config.type
		? TABS[config.type]
		: ["Describe", "Content", "Build", "Test"];
	...
	const addKnowledge = () => {
		setConfig((c) => ({
			...c,
			knowledge: [...c.knowledge, `Document_${c.knowledge.length + 1}.pdf`],
		}));
	};
	...
	const removeAction = (action: string) => {
		setConfig((c) => ({
			...c,
			actions: c.actions.filter((a) => a !== action),
		}));
	};
						...
						<div className="flex items-center gap-3 mb-2">
							...
							<div>
								...
								<p className="text-sm text-slate-500">
									Give it a name and choose the type
								</p>
							</div>
						</div>
					</div>
	...
	return (
		<div className="flex flex-col h-full bg-slate-100">
						...
						{activeTab === "Describe" && (
							<div className="space-y-6">
								<div>
									...
									<p className="text-sm text-slate-500">
										Define how this agent behaves and what it does.
									</p>
								</div>
								...
								<Card className="p-5 space-y-5">
									...
									<div>
										...
										<textarea
											value={config.goal}
											onChange={(e) =>
												setConfig((c) => ({ ...c, goal: e.target.value }))
											}
									...
									<div>
										...
										<textarea
											...
											onChange={(e) =>
												setConfig((c) => ({ ...c, completion: e.target.value }))
											}
						...
						{activeTab === "Content" && config.type !== "workflow_executor" && (
							<div className="space-y-6">
								<div>
									...
									<p className="text-sm text-slate-500">
										Add documents or connect sources for this agent to
										reference.
									</p>
								</div>
									...
									{config.actions.length > 0 && (
										<div className="space-y-2 mb-4">
											...
											{config.actions.map((a, i) => (
												<div
													key={i}
													className="flex items-center justify-between p-3 bg-violet-50 rounded-lg border border-violet-200"
												>
								...
								{config.actions.length > 0 && (
									<Card className="p-5">
										...
										<div className="flex items-center gap-2 flex-wrap">
											...
											<span className="text-slate-400">→</span>
											<span className="px-3 py-1.5 bg-slate-200 text-slate-600 rounded-lg text-sm font-medium">
												End
											</span>
										</div>
									</Card>
						...
						{activeTab === "Build" && (
							<div className="space-y-6">
								<div>
									...
									<p className="text-sm text-slate-500">
										Configure how the agent responds and handles edge cases.
									</p>
								</div>
								...
								<Card className="p-5 space-y-5">
									...
									<div>
										...
										<select
											...
											onChange={(e) =>
												setConfig((c) => ({ ...c, fallback: e.target.value }))
											}
											className="w-full px-4 py-3 border rounded-lg bg-slate-50 focus:ring-2 focus:ring-violet-500 focus:border-violet-500"
										>
											<option value="apologize">
						...
						{activeTab === "Test" && (
							<div className="space-y-6">
								<div>
									...
									<p className="text-sm text-slate-500">
										Try out conversations in the preview panel on the right.
									</p>
								</div>
								...
								<Card className="p-5">
									<div className="text-center py-12">
										...
										<p className="text-sm text-slate-500 mt-1">
											Use the preview panel to simulate conversations
										</p>
									</div>
								</Card>
						...
						{previewMessages.length === 0 ? (
							<div className="h-full flex items-center justify-center">
								<div className="text-center px-4">
									{selectedType && (
										<div
											className={`w-14 h-14 ${selectedType.color} rounded-2xl flex items-center justify-center mx-auto mb-3`}
										>
			...
			<div
				className={`fixed bottom-4 left-1/2 -translate-x-1/2 w-full max-w-xl transition-all ${aiChatOpen ? "h-72" : "h-auto"}`}
			>
