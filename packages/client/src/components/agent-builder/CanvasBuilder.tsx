...
import type { AgentDefinition, StepType, WorkflowStep } from "./types";
				...
				<div className="flex-1 overflow-auto p-2 space-y-1">
					{(
						Object.entries(STEP_TYPE_META) as [
							StepType,
							(typeof STEP_TYPE_META)[StepType],
						][]
					).map(([type, meta]) => (
						<div
							key={type}
			...
			>
				...
				{steps.map((step) => (
					<StepNode
						...
						onConnect={(targetId, label) =>
							handleConnect(step.id, targetId, label)
						}
						allSteps={steps}
					/>
				))}
					...
					<div className="flex-1 overflow-auto p-4 space-y-4">
						<div>
							...
							<input
								...
								onChange={(e) =>
									handleUpdateStep(selectedStep.id, { name: e.target.value })
								}
							/>
						</div>
