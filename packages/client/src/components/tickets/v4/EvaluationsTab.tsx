...
export function EvaluationsTab({
	...
	if (selectedEval) {
		...
		const passCount = selectedEval.cases.filter((c) => c.score === "pass").length;
		const failCount = selectedEval.cases.filter((c) => c.score === "fail").length;
			...
			<div className="flex flex-col lg:flex-row gap-0 -mx-4 -mb-4">
				...
				<div className="flex-1 min-w-0 px-4 pb-4 flex flex-col gap-3">
					...
					<div className="flex items-center gap-1 text-xs">
						...
						<span className="text-foreground font-medium">{selectedEval.name}</span>
					</div>
			...
			<div className="flex items-start justify-between gap-4">
				<div>
					...
					<p className="text-xs text-muted-foreground mt-0.5">
						Test how well <span className="font-medium">{agent.name}</span> handles
						tickets in this cluster. Run a batch and review pass/fail scores.
					</p>
				</div>
				...
				{ordered.length === 0 ? (
					<div className="rounded-lg border border-dashed bg-muted/20 p-6 text-center">
						...
						<p className="text-xs text-muted-foreground mb-3 max-w-sm mx-auto">
							No evaluations yet. Run a batch of tickets through the agent to see
							pass/fail scores.
						</p>
						<Button
							size="sm"
