...
import { Settings, RotateCcw, Cloud } from 'lucide-react'
...
export function FeatureFlagsPanel() {
  ...
  const handlePlatformToggle = async (key: FeatureFlagKey, checked: boolean) => {
    ...
    const results = await Promise.all(
      flagsToUpdate.map((flag) => setPlatformFlag(flag, flag === key ? checked : false))
    )
  ...
  const renderPlatformFlag = (key: FeatureFlagKey, disabled?: boolean) => {
    ...
    return (
      <div key={key} className={`flex items-start justify-between gap-4 ${disabled ? 'opacity-50' : ''}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          {isUpdating && <span className="text-sm text-muted-foreground/50">Saving...</span>}
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      {categories.general.length > 0 && (
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {categories.general.map(renderLocalFlag)}
          </div>
        </Card>
      )}
      ...
      {categories.debug.length > 0 && (
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {categories.debug.map(renderLocalFlag)}
          </div>
        </Card>
      )}

      {/* Experimental Features (includes Auto Pilot) */}
      {(categories.experimental.length > 0 || categories.autopilot.length > 0) && (
        <Card className="p-6 border-amber-200 dark:border-amber-900 bg-amber-50/50 dark:bg-amber-950/20">
          ...
          {categories.experimental.length > 0 && (
            <>
              ...
              <div className="space-y-4">
                {categories.experimental.map(renderLocalFlag)}
              </div>
            </>
          )}
