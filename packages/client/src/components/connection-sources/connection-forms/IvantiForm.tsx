...
export interface IvantiFormData {
	url: string;
	apiKey: string;
}

interface IvantiFormProps {
	onCancel?: () => void;
	onSuccess?: () => void;
	onFailure?: () => void;
}

export function IvantiForm({
	onCancel,
	onSuccess,
	onFailure,
}: IvantiFormProps = {}) {
	...
	const handleConnect = async () => {
		...
		try {
			...
			ritaToast.success({
				...
				description: tToast("descriptions.ivantiConfigured"),
			});
			...
			<FormSection title={t("form.sections.authentication")}>
				...
				>
					<Input
						id="ivanti-url"
						type="url"
						placeholder="https://your-instance.trysaasit.com"
						{...register("url", {
