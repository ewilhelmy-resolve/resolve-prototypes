			...
			<div className="p-6">
				<div className="mb-6">
					...
					<p className="text-sm text-muted-foreground mt-1">
						Configure the LLM endpoint used by Resolve for task analysis and
						script generation.
					</p>
				</div>
					...
					<section className="space-y-4 rounded-lg border p-5 self-start">
						...
						<div className="space-y-1.5">
							...
							<p className="text-[11px] text-muted-foreground">
								Full endpoint URL for the LLM API. Must be reachable from the
								Resolve control node.
							</p>
						</div>
						...
						<div className="space-y-1.5">
							...
							/>
							<p className="text-[11px] text-muted-foreground">
								Required if the provider restricts access by source IP or uses a
								secondary token.
							</p>
						</div>
						...
						{testResult.status === "idle" && (
							<div className="rounded-lg border border-dashed p-8 flex flex-col items-center justify-center text-center gap-2">
								...
								<p className="text-sm text-muted-foreground">
									Enter your LLM URL and token, then run a test to see results
									here.
								</p>
							</div>
						)}
						...
						{testResult.status === "fail" && (
							<div className="rounded-lg border border-destructive/30 bg-destructive/5 p-5 space-y-2">
								...
								<p className="text-xs text-muted-foreground">
									Check that the URL is reachable and the auth token is valid.
							...
							<div className="space-y-4">
								...
								{testResult.groovyScript && (
									<section className="rounded-lg border p-5 space-y-3">
										...
										<p className="text-[11px] text-muted-foreground">
											This script parses the LLM response for downstream Resolve
											tasks. The {testResult.matchScore}% match score indicates
											how well the generated script covers the expected output
											schema.
										</p>
									</section>
								)}
