...
import { ArrowLeft, Search, MoreHorizontal, X, Hourglass, Calendar, ExternalLink } from 'lucide-react';
...
import { MOCK_SCHEDULED_WORKFLOWS, WORKFLOW_GROUPS, WORKFLOW_GROUP_MAP, getMockWorkflowDetail } from '@/data/mock-scheduler';
...
export default function SchedulerGroupDetailPage() {
  ...
  const groupWorkflows = useMemo(() => {
    return MOCK_SCHEDULED_WORKFLOWS.filter(w => WORKFLOW_GROUP_MAP[w.id] === groupId);
  }, [groupId]);
  ...
  const stats = useMemo(() => {
    ...
    const totalSuccess = groupWorkflows.reduce((sum, w) => sum + w.successCount, 0);
    const successRate = totalRuns > 0 ? Math.round((totalSuccess / totalRuns) * 100) : 100;
    const healthy = groupWorkflows.filter(w => calculateSuccessRate(w) >= 90).length;
    const needsAttention = groupWorkflows.filter(w => calculateSuccessRate(w) < 90).length;
    ...
    return (
      <RitaLayout activePage="scheduler">
        <div className="flex flex-col items-center justify-center h-[60vh]">
          ...
          <Button variant="ghost" onClick={() => navigate('/scheduler')} className="mt-4">
      ...
      <div className="flex flex-col h-[calc(100vh-64px)] bg-background">
        ...
        <div className="px-6 py-4">
          <div className="grid grid-cols-4 gap-3">
            <StatCard value={stats.totalRuns.toLocaleString()} label="Total executions" />
            <StatCard value={`${stats.successRate}%`} label="Success rate" />
            <StatCard value={stats.healthy.toString()} label="Healthy workflows" />
            <StatCard value={stats.needsAttention.toString()} label="Needs Attention" />
          </div>
        </div>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => navigate(`/scheduler/${workflow.id}`)}>
                          View Details
                        </DropdownMenuItem>
        ...
        <Sheet open={!!selectedWorkflowId} onOpenChange={(open) => !open && setSelectedWorkflowId(null)}>
              ...
              <div className="flex flex-col h-full">
                ...
                <div className="p-4 border-b space-y-2">
                  <div className="flex items-start justify-between">
                    ...
                    >
                      {selectedWorkflow.status === 'enabled' ? 'Active' : 'Paused'}
                    </Badge>
                    <Button
                      variant="ghost"
                      size="icon"
                  ...
                  <h2 className="text-lg font-heading">{selectedWorkflow.name}</h2>
                ...
                <div className="p-4 border-b grid grid-cols-2 gap-4">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm">{formatDate(selectedWorkflow.nextRunTime)}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm">{formatDate(selectedWorkflow.lastRunTime)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 border-b grid grid-cols-3 gap-4">
                  <div>
                    <p className="text-2xl font-heading">{selectedWorkflow.totalRuns.toLocaleString()}</p>
                    <p className="text-xs text-muted-foreground">Total runs</p>
                  </div>
                  <div>
                    <p className="text-2xl font-heading">{selectedWorkflow.successCount.toLocaleString()}</p>
                    <p className="text-xs text-muted-foreground">Successful runs</p>
                  </div>
                  <div>
                    <p className="text-2xl font-heading">{selectedWorkflow.failureCount.toLocaleString()}</p>
                    ...
                    <TableBody>
                      {selectedWorkflow.executions.slice(0, 10).map((exec) => (
                        <TableRow key={exec.id}>
                          ...
                          <TableCell className="text-xs text-muted-foreground">
                            {formatDate(selectedWorkflow.nextRunTime).split(' ').slice(0, 2).join(' ')}...
                          </TableCell>
                          <TableCell>
                            {exec.logsUrl && (
                              <a
