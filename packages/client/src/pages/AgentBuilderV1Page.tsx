...
export default function AgentBuilderV1Page() {
	...
	const [viewState, setViewState] = useState<"form" | "building" | "success" | "error">("form");
	const [buildSteps, setBuildSteps] = useState<Array<{ label: string; description: string; status: "pending" | "active" | "complete" | "error" }>>([]);
	const [buildError, setBuildError] = useState<string | null>(null);
	const buildTimersRef = useRef<NodeJS.Timeout[]>([]);

	const simulateFailure = new URLSearchParams(location.search).get("fail") === "true";
	...
	const handleCreateAgent = () => {
		...
		const stepsToRun = failAtStep >= 0 ? BUILD_STEP_SEQUENCE.slice(0, failAtStep + 1) : BUILD_STEP_SEQUENCE;

		stepsToRun.forEach((step, index) => {
			const t = setTimeout(() => {
				setBuildSteps((prev) => {
					const updated = prev.map((s) => ({ ...s, status: "complete" as const }));
					return [...updated, { ...step, status: index === failAtStep ? "error" as const : "active" as const }];
				});
			}, (index + 1) * stepDelay);
		...
		if (failAtStep >= 0) {
			const errorT = setTimeout(() => {
				setBuildError("Failed to create agent tasks. The knowledge source connection timed out. Please check your configuration and try again.");
				setViewState("error");
				...
				<div className="flex-1 flex items-center justify-center">
					<div className="max-w-md w-full px-6">
						<div className="text-center mb-8">
							...
							<h1 className="text-2xl font-semibold text-foreground">Agent creation failed</h1>
							<p className="text-muted-foreground mt-2">Something went wrong while building your agent</p>
						</div>
						...
						<div className="space-y-1 mb-8">
							{buildSteps.map((step, index) => (
								<div key={index} className="flex items-start gap-3 py-2">
									...
									<div>
										<p className={cn("text-sm font-semibold", step.status === "error" ? "text-red-600" : "text-foreground")}>{step.label}</p>
										<p className={cn("text-sm", step.status === "error" ? "text-red-500" : "text-muted-foreground")}>{step.status === "error" ? "Failed" : step.description}</p>
									</div>
								</div>
							))}
						</div>
		...
		<div className="h-full flex flex-col bg-muted/40">
								...
								<div>
									<div className="flex items-center justify-between">
										...
										<Button
											...
											disabled={isImprovingInstructions || !!improvePreview || !config.instructions.trim()}
											onClick={async () => {
									...
									<div>
										<div className="flex items-center gap-1.5">
											...
											<FieldHelpPopover
												...
												examples={["How do I request PTO?", "Reset my password"]}
			...
			{(() => {
				const rows = improvePreview
					? computeSideBySideRows(improvePreview.original, improvePreview.improved)
					: [];
				return (
					<Dialog
						open={!!improvePreview}
					...
					>
						<DialogContent className="!max-w-6xl max-h-[85vh] overflow-hidden flex flex-col">
							<DialogHeader>
								...
								<p className="text-sm text-muted-foreground">Review the suggested changes before accepting.</p>
							</DialogHeader>
							<div className="overflow-y-auto rounded-lg max-h-[60vh]">
								...
								<div className="grid grid-cols-2 gap-4 sticky top-0 z-10 bg-background pb-2">
									...
									<p className="text-sm font-medium text-muted-foreground">Improved</p>
								</div>
										...
										{rows.map((row, i) => (
											<div
												...
												className={cn(
													...
													row.left.type === "removed" && "bg-red-50 text-red-900",
													row.left.type === "unchanged" && "text-foreground",
													row.left.type === "empty" && "bg-muted/10",
												)}
											>
												{row.left.type !== "empty" ? (row.left.text || "\u00A0") : "\u00A0"}
											</div>
										))}
									</div>
									<div className="rounded-lg overflow-hidden border">
										{rows.map((row, i) => (
											<div
												...
												className={cn(
													"px-4 py-0.5 text-sm min-h-[26px]",
													row.right.type === "added" && "bg-green-50 text-green-900",
													row.right.type === "unchanged" && "text-foreground",
													row.right.type === "empty" && "bg-muted/10",
												)}
											>
												{row.right.type !== "empty" ? (row.right.text || "\u00A0") : "\u00A0"}
											</div>
										))}
									</div>
								</div>
