...
const TASK_CONFIGS: TaskConfig[] = [
	...
	},
	{
		...
		params: [
			...
			{
				name: "NOKIA_5620_SAM_URL",
				source: "DEFAULT",
				value: "$PROPERTY(NOKIA_5620_SAM_URL)",
			},
			{
				name: "NOKIA_5620_SAM_USERNAME",
				source: "DEFAULT",
				value: "$PROPERTY(NOKIA_5620_SAM_USERNAME)",
			},
			{
				name: "NOKIA_5620_SAM_PASSWORD",
				source: "DEFAULT",
				value: "$PROPERTY(NOKIA_5620_SAM_PASSWORD)",
			},
		],
	},
	{
		...
		params: [
			{
				name: "EQUIPMENT_NAME",
				source: "DEFAULT",
				value: "$OUTPUT(get_site.equipmentName)",
			},
									...
									<p
										...
										dangerouslySetInnerHTML={{
											__html: msg.content
												...
												.replace(
													/`(.*?)`/g,
													"<code class='bg-blue-100 px-1 rounded text-xs'>$1</code>",
												)
												.replace(/• /g, "• "),
										}}
									/>
...
function StartNode({ x, y }: { x: number; y: number }) {
	return (
		<g transform={`translate(${x}, ${y})`}>
			...
			<text y={32} textAnchor="middle" fontSize={11} fill="#334155">
				Start
			</text>
		</g>
	);
...
function TaskNode({
	...
	return (
		...
		>
			...
			{sublabel && (
				<text x={-w / 2 + 44} y={13} fontSize={10} fill="#64748b">
					{sublabel}
				</text>
			)}
		</g>
	...
	return (
		<g transform={`translate(${x}, ${y})`}>
			<polygon
				...
				strokeWidth={1.5}
			/>
			<text x={24} y={5} fontSize={11} fill="#334155">
				{label}
			</text>
		</g>
	);
...
function CommentNode({
	...
	return (
		<g transform={`translate(${x}, ${y})`}>
			...
			{sublabel && (
				<text x={-w / 2 + 40} y={10} fontSize={9} fill="#64748b">
					{sublabel}
				</text>
			)}
		</g>
...
function GroupRedEdge({ x, y1, y2 }: { x: number; y1: number; y2: number }) {
	return (
		<line x1={x} y1={y1} x2={x} y2={y2} stroke="#ef4444" strokeWidth={2} />
	);
}
						...
						<div className="flex-1 overflow-auto p-6">
							...
							>
								...
								<DecisionNode
									x={300}
									y={282}
									label="Does Name of Loopback IP match Equipment Name?"
								/>
								...
								<text x={110} y={435} fontSize={11} fill="#475569">
									Check if Port is Available, and if other L3 Services already
									exist
								</text>
								<line
									x1={80}
								...
								<DecisionNode
									x={300}
									y={552}
									label="Confirm port is spare and available"
								/>
								...
								<text
									...
									fill="#94a3b8"
								>
									You have the option of RESOLVE connecting directly to the
									target device to get the required information,
								</text>
								<text
									x={340}
									y={744}
									...
									fill="#94a3b8"
								>
									or utilizing the Nokia 5620 SAM to execute the necessary
									commands remotely (because RESOLVE doesn't have direct network
									connectivity).
								</text>
							</svg>
						</div>
						...
						<div className="flex items-center px-4 py-2 border-b">
							...
							<div className="ml-auto">
								<button
									type="button"
									onClick={() =>
										setPropsTab(propsTab === "jarvis" ? "configure" : "jarvis")
									}
