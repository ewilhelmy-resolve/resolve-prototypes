...
export const SOURCE_IDS = ['confluence', 'sharepoint', 'servicenow', 'websearch'] as const;
...
export function getDisplayStatus(
  ...
  // Currently verifying
  if (status === 'verifying') {
    return STATUS.SYNCING; // Show as "Syncing" for now (could add separate "Verifying" status)
  }
  ...
  switch (source.type) {
    case 'confluence':
      ...
      if (source.settings?.spaces) {
        const spaces = typeof source.settings.spaces === 'string'
          ? source.settings.spaces.split(',').map(s => s.trim()).filter(Boolean)
 ...
 */
export function mapDataSourceToUI(source: DataSourceConnection): ConnectionSource {
