...
export async function fetchPlatformFlag(
	...
	try {
		const response = await fetch(
			`${API_BASE_URL}/api/feature-flags/${clientKey}`,
			{
				method: "GET",
				credentials: "include",
				headers: {
		...
		if (!response.ok) {
			console.warn(
				`Platform flag fetch failed for ${clientKey}: ${response.status}`,
			);
			return false;
		}
...
export async function updatePlatformFlag(
	...
	try {
		const response = await fetch(
			`${API_BASE_URL}/api/feature-flags/${clientKey}/rules`,
			{
				method: "POST",
				credentials: "include",
				headers: {
		...
		if (!response.ok) {
			console.warn(
				`Platform flag update failed for ${clientKey}: ${response.status}`,
			);
			return false;
		}
