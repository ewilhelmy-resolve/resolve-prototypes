...
import { type DynamicWorkflowEvent, getSSEService } from "../services/sse.js";
...
export class WorkflowConsumer {
	...
	async startConsumer(channel: Channel): Promise<void> {
		...
		await channel.consume(
			...
			async (message: ConsumeMessage | null) => {
				...
				try {
					const content: WorkflowMessage = JSON.parse(
	...
	private async processWorkflowMessage(
		...
		if (!tenant_id || !user_id || !action) {
			...
			messageLogger.error(
				{ payload },
				"Invalid workflow payload: missing required fields",
			);
		...
		try {
			...
			messageLogger.info(
				{ eventType: "dynamic_workflow", action },
