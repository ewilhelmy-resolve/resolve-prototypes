...
// Success message
interface DocumentProcessingCompletedMessage
	extends DocumentProcessingStatusMessage {
	status: "processing_completed";
	processed_markdown?: string;
}

// Failure message
interface DocumentProcessingFailedMessage
	extends DocumentProcessingStatusMessage {
...
async function withRetry<T>(
	...
	throw new Error(
		`Document not found after ${RETRY_CONFIG.maxRetries} retries`,
	);
}
...
export class DocumentProcessingConsumer {
	...
	constructor() {
		this.queueName =
			process.env.DOCUMENT_PROCESSING_QUEUE || "document_processing_status";
	}
	...
	async startConsumer(channel: Channel): Promise<void> {
		queueLogger.info(
			...
			"Starting Document Processing consumer...",
		);
		...
		await channel.consume(
			...
			async (message: ConsumeMessage | null) => {
				...
				try {
					const content: DocumentProcessingMessage = JSON.parse(
					...
					logError(queueLogger, error as Error, {
						operation: "document-processing-status",
					});
		...
		queueLogger.info(
			{ queueName: this.queueName },
			"Document Processing consumer started successfully",
		);
	}
	 ...
	 */
	private async processDocumentStatus(
		...
		if (!blob_metadata_id || !tenant_id || !status) {
			...
			messageLogger.error(
				{ payload },
				"Invalid document processing payload: missing required fields",
			);
			throw new Error(
				"Invalid document processing payload: missing required fields",
			);
		...
		switch (status) {
			case "processing_completed":
				await this.handleProcessingCompleted(
					payload as DocumentProcessingCompletedMessage,
					messageLogger,
				);
				break;
			...
			case "processing_failed":
				await this.handleProcessingFailed(
					payload as DocumentProcessingFailedMessage,
					messageLogger,
				);
				break;
            ...
            `,
						...
						if (doc.status === "processed") {
							messageLogger.info(
								...
								"Document already processed, skipping (idempotent)",
							);
		...
		const result = await withRetry(
            ...
            `,
						...
						if (doc.status === "processed" || doc.status === "failed") {
							messageLogger.info(
								{ currentStatus: doc.status },
								"Document already in final state, skipping (idempotent)",
							);
							return { ...doc, skipped: true };
						}
            ...
            `,
							[
								payload.error_message || "Processing failed",
								payload.blob_metadata_id,
								payload.tenant_id,
							],
						);
