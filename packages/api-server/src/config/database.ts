...
export const withOrgContext = async <T>(
  ...
  contextLogger.debug('Starting database transaction with organization context');
  ...
  try {
    ...
    logError(contextLogger, error instanceof Error ? error : new Error(String(error)), {
      userId,
      organizationId,
      operation: 'withOrgContext'
    });
    ...
    try {
      ...
      logError(contextLogger, resetError instanceof Error ? resetError : new Error(String(resetError)), {
