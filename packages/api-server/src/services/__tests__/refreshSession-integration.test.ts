 ...
 *
 * Skipped automatically when Valkey is not available (CI without Valkey service).
 * Run locally: VALKEY_URL=redis://localhost:6379 pnpm vitest run src/services/__tests__/refreshSession-integration.test.ts
 */
...
// Check if Valkey is reachable before running tests
...
try {
	...
	// Valkey not available — tests will be skipped
}

// Direct Redis client for test setup/teardown
let testRedis: Redis;
...
describe.skipIf(!valkeyAvailable)(
	"refreshSessionFromValkey - real Valkey integration",
	() => {
		beforeAll(async () => {
		...
		describe("Scenario 1: Actions Platform writes initial payload, Rita reads it", () => {
			it("should read initial payload from Valkey via HGET", async () => {
				...
				expect((parsed.context as Record<string, unknown>).activityId).toBe(
					9999,
				);
				expect(
					(parsed.context as Record<string, unknown>).runId,
				).toBeUndefined(); // not yet added
			});
		});
		...
		describe("Scenario 2: Actions Platform updates context mid-session (adds runId)", () => {
			it("should see updated context after Actions Platform writes runId", async () => {
				...
				expect(
					(read1.context as Record<string, unknown>).runId,
				).toBeUndefined();
