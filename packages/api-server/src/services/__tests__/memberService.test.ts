...
describe("MemberService - Critical Business Rules", () => {
	...
	describe("Sorting - Stable sort with tiebreaker", () => {
		const orgId = "org-123";

		const setupListMembersQuery = () => {
			// Mock: SELECT members query
			(mockPool.query as any).mockResolvedValueOnce({
				rows: [],
				rowCount: 0,
			});
		...
		it("should include a tiebreaker column in ORDER BY when sorting by role", async () => {
			setupListMembersQuery();

			await memberService.listMembers(orgId, {
				sortBy: "role",
				sortOrder: "asc",
			});

			const selectQuery = (mockPool.query as any).mock.calls[0][0];
			// Should have tiebreaker: ORDER BY om.role ASC, om.user_id ASC
			expect(selectQuery).toMatch(
				/ORDER BY\s+om\.role\s+ASC\s*,\s*om\.user_id/i,
			);
		});

		it("should include a tiebreaker column in ORDER BY when sorting by status", async () => {
			setupListMembersQuery();
			...
			const selectQuery = (mockPool.query as any).mock.calls[0][0];
			expect(selectQuery).toMatch(/,\s*om\.user_id/i);
		});

		it("should include a tiebreaker for all sort fields", async () => {
			const sortFields: Array<
				"name" | "role" | "status" | "conversationsCount" | "joinedAt"
			> = ["name", "role", "status", "conversationsCount", "joinedAt"];
