...
describe("WebhookService", () => {
	 ...
	 */
	describe("sendMessageEvent - RITA (includes document_ids, transcript_ids)", () => {
		it("should include document_ids and transcript_ids in payload", async () => {
			...
			await webhookService.sendMessageEvent({
				...
				conversationId: "conv-789",
				messageId: "msg-101",
				customerMessage: "Hello from RITA",
				documentIds: ["doc-1", "doc-2"],
			...
			expect(calledPayload.transcript_ids).toEqual({
				transcripts: [{ role: "user", content: "Hello" }],
			});
			expect(calledPayload.customer_message).toBe("Hello from RITA");
	...
	describe("sendTenantMessageEvent - Valkey camelCase to webhook snake_case routing", () => {
		...
		it("should include snake_case routing fields for RabbitMQ", async () => {
			...
			// REQUIRED snake_case fields for RabbitMQ routing
			expect(calledPayload.tenant_id).toBe(
				"00F4F67D-3B92-4FD2-A574-7BE22C6BE796",
			);
			expect(calledPayload.user_guid).toBe(
		...
		it("should preserve camelCase fields from Valkey for Actions API", async () => {
			...
			expect(calledPayload.tenantId).toBe(
				"00F4F67D-3B92-4FD2-A574-7BE22C6BE796",
			);
			expect(calledPayload.userGuid).toBe(
				"472e6672-b1f5-4ee4-86d8-3804eb8d064d",
			);
			expect(calledPayload.chatSessionId).toBe(
				"b08c2f5c-2071-4e69-b753-dd381c41bf64",
			);
			expect(calledPayload.tabInstanceId).toBe(
				"62ea2274-8ecc-4390-9c16-2a007cd97850",
			);
			expect(calledPayload.actionsApiBaseUrl).toBe(
				"https://actions-api-staging.resolve.io/",
			);
			expect(calledPayload.context).toEqual({
				designer: "activity",
				activityId: 2209,
			});
		});
		...
		it("should include uiConfig in camelCase for Actions API", async () => {
			...
			expect(uiConfig.welcomeText).toBe(
				"Start by just describing in detail what you want.",
			);
			expect(uiConfig.placeholderText).toBe(
		...
		it("should call tenant-specific webhook URL with Basic auth", async () => {
			...
			const expectedAuth = Buffer.from(
				`${canonicalIframeConfig.clientId}:${canonicalIframeConfig.clientKey}`,
			).toString("base64");
