...
describe('WorkflowExecutionService', () => {
  ...
  describe('getConfigFromValkey', () => {
    it('should fetch and parse valid config from Valkey', async () => {
      ...
      const { config, debug } = await service.getConfigFromValkey('hashkey-123');
    ...
    it('should return null config when key not found', async () => {
      ...
      const { config, debug } = await service.getConfigFromValkey('missing-key');
    ...
    it('should return null config on JSON parse error', async () => {
      ...
      const { config, debug } = await service.getConfigFromValkey('bad-json-key');
    ...
    it('should return null config when required fields missing', async () => {
      ...
      const { config, debug } = await service.getConfigFromValkey('incomplete-key');
    ...
    it('should include access tokens in payload', async () => {
      ...
      expect(mockedAxios.post).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          token_expiry: validValkeyConfig.tokenExpiry,
        }),
        expect.any(Object)
      );
    });
