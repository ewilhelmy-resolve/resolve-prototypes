...
describe("InvitationService", () => {
	...
	describe("sendInvitations", () => {
		...
		it("should successfully send invitations to valid emails", async () => {
			...
			vi.mocked(mockPool.query).mockResolvedValueOnce({
				...
				rowCount: 0,
				fields: [],
			} as any); // existing invitation check
		...
		it("should skip users who are already members (INV006)", async () => {
			...
			expect(result.invitations[0].reason).toBe(
		...
		it("should handle webhook failures and mark invitations as failed", async () => {
			...
			expect(result.invitations[0].reason).toBe(
				"Webhook failed to send invitation emails",
			);
		});
		...
		it("should generate 64-character hex tokens", async () => {
			...
			await invitationService.sendInvitations(organizationId, invitedByUserId, [
				"test@example.com",
			]);

			// Check webhook was called with proper token format
			const webhookCall = vi.mocked(mockWebhookService.sendGenericEvent).mock
				.calls[0][0];
	...
	describe("listInvitations", () => {
		...
		it("should filter by status", async () => {
			...
			await invitationService.listInvitations(organizationId, {
				status: "pending",
			});
