...
const { mockPool, mockWithOrgContext, mockSSEService } = vi.hoisted(() => ({
	mockPool: {
		connect: vi.fn(),
		query: vi.fn(),
	},
					...
					const mockOrgClient = {
						query: vi
							...
							.mockResolvedValueOnce({
								rows: [{ id: productionPayload.message_id }],
							}) // message check
				...
				async (userId, _orgId, callback) => {
					...
					const mockOrgClient = {
						query: vi
							.fn()
							.mockResolvedValueOnce({
								rows: [{ id: newUserPayload.message_id }],
							})
