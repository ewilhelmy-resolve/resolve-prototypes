...
type ConnectionStatus = 'disconnected' | 'connecting' | 'connected' | 'reconnecting' | 'circuit_open';
...
export class RabbitMQService {
  ...
  constructor() {
    ...
    this.retryConfig = {
      maxAttempts: Number.parseInt(process.env.RABBITMQ_RETRY_MAX_ATTEMPTS || '10', 10),
      initialDelayMs: Number.parseInt(process.env.RABBITMQ_RETRY_INITIAL_DELAY_MS || '1000', 10),
      maxDelayMs: Number.parseInt(process.env.RABBITMQ_RETRY_MAX_DELAY_MS || '32000', 10),
      backoffMultiplier: Number.parseFloat(process.env.RABBITMQ_RETRY_BACKOFF_MULTIPLIER || '2'),
      jitterEnabled: process.env.RABBITMQ_RETRY_JITTER_ENABLED !== 'false',
    };

    queueLogger.info({ retryConfig: this.retryConfig }, 'RabbitMQ service initialized with retry configuration');
  }
  ...
  private setupEventListeners(): void {
    ...
    this.connection.on('close', () => {
      queueLogger.warn({
        ...
        uptimeMs: this.state.lastConnectedAt ? Date.now() - this.state.lastConnectedAt.getTime() : 0,
        state: this.state.status,
  ...
  private async reconnect(): Promise<void> {
    ...
    if (this.retryConfig.maxAttempts > 0 && this.state.reconnectAttempts >= this.retryConfig.maxAttempts) {
      queueLogger.error({
      ...
      try {
        ...
        if (this.state.status === 'connected') {
          ...
          queueLogger.info({
            ...
            attempt: this.state.reconnectAttempts,
            downtimeMs: this.state.lastErrorAt ? Date.now() - this.state.lastErrorAt.getTime() : 0,
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      queueLogger.info({
        ...
        queueName: this.queueName,
        wasReconnecting,
        downtimeMs: wasReconnecting && this.state.lastErrorAt
