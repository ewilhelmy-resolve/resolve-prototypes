...
export class WebhookService {
	...
	async sendTenantMessageEvent(params: {
		...
		if (
			!iframeConfig.actionsApiBaseUrl ||
			!iframeConfig.clientId ||
			!iframeConfig.clientKey
		) {
			throw new Error(
				"Missing required webhook credentials (actionsApiBaseUrl, clientId, clientKey)",
			);
		}
	...
	private async sendEventToUrl(
		...
		for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
			try {
				console.log(
					`[WebhookService] Sending tenant event (attempt ${attempt}/${this.config.retryAttempts}):`,
					{
						url,
				...
				console.log(
					`[WebhookService] Tenant webhook success: ${response.status}`,
				);
				...
				console.error(
					`[WebhookService] Tenant webhook attempt ${attempt} failed:`,
					{
						url,
		...
		console.error(
			`[WebhookService] All tenant webhook attempts failed for ${payload.action}`,
		);
	...
	private async sendEvent(payload: WebhookPayload): Promise<WebhookResponse> {
		...
		for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
			try {
				console.log(
					`[WebhookService] Sending event (attempt ${attempt}/${this.config.retryAttempts}):`,
					{
						source: payload.source,
						action: payload.action,
						tenant_id: payload.tenant_id,
	...
	private async storeWebhookFailure(
        ...
        `,
			...
			console.error(
				`[WebhookService] Failed to store webhook failure in database:`,
				{
					error:
	...
	async trackAction(params: {
		...
		console.log(
			`[WebhookService] Tracking action: ${params.action} from ${params.source}`,
			{
				user_email: params.userEmail,
