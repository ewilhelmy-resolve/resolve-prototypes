...
export class MemberService {
		...
		const sortColumn =
			sortBy === "name"
				...
				: sortBy === "role"
					...
					: sortBy === "status"
						? "om.is_active"
						: sortBy === "conversationsCount"
							? "conversations_count"
							: "om.joined_at";
		query += ` ORDER BY ${sortColumn} ${sortOrder.toUpperCase()}, om.user_id ASC`;
	...
	async updateMemberStatus(
		...
		try {
			...
			if (!canPerform) {
				throw new Error(
					"Permission denied: You cannot change this member's status",
				);
			}
	...
	async canPerformAction(
		...
		const { performer_role: performerRole, target_role: targetRole } =
			result.rows[0];
	...
	async deleteMemberPermanent(
		...
		try {
			...
			if (!canPerform) {
				throw new Error(
					"Permission denied: Only owners can permanently delete members",
				);
			}
         ...
         WHERE om.organization_id = $1 AND up.user_id = $2`,
			...
			if (!webhookResponse.success) {
				throw new Error(
					`Keycloak deletion failed: ${webhookResponse.error || "Unknown error"}`,
				);
			}
         ...
         VALUES ($1, $2, $3, $4, $5, $6)`,
			...
			await client.query("DELETE FROM pending_users WHERE email = $1", [
				member.email,
			]);
         ...
         WHERE up.user_id = $1`,
           ...
           WHERE om.organization_id = $1`,
				...
				if (user.role === "owner") {
					...
					const isLastOwner = await this.isLastActiveOwner(
						user.organization_id,
						userId,
					);
			...
			logger.info(
				{
					...
					totalMembersToDelete: deleteEntireOrganization
						? allOrgMembers.length
						: 1,
					additionalEmails,
				},
			...
			const webhookResponse = await webhookService.deleteKeycloakUser({
				...
				reason:
					reason ||
					(deleteEntireOrganization
						? "Owner deleted account - organization deleted"
         ...
         VALUES ($1, $2, $3, $4, $5, $6)`,
				[
					...
					JSON.stringify({
						...
						totalMembersDeleted: deleteEntireOrganization
							? allOrgMembers.length
							: 1,
						memberEmails: deleteEntireOrganization
							? allOrgMembers.map((m) => m.email)
							: [user.email],
					}),
				],
			);
