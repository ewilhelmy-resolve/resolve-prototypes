...
import type { CreateDelegationRequest, ListDelegationsQuery, SubmitCredentialsRequest } from '../types/credentialDelegation.js';
 ...
 */
router.post('/create', authenticateUser, requireRole(['owner', 'admin']), async (req, res) => {
  const authReq = req as AuthenticatedRequest;
  try {
    const { admin_email, itsm_system_type } = req.body as CreateDelegationRequest;
...
router.post('/submit', async (req, res) => {
  try {
    ...
    const result = await credentialDelegationService.submitCredentials(token, credentials);
...
router.get('/', authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const delegations = await credentialDelegationService.listDelegations(organizationId, query);
...
router.delete('/:id/cancel', authenticateUser, requireRole(['owner', 'admin']), async (req, res) => {
  ...
  try {
    ...
    const result = await credentialDelegationService.cancelDelegation(id, organizationId, userId);

    logger.info({ delegationId: id, organizationId, userId }, 'Delegation cancelled');
    ...
    if (error instanceof Error && error.message.includes('not found')) {
      return res.status(404).json({ error: 'Delegation not found or cannot be cancelled' });
    }
