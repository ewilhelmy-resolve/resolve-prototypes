...
describe("Transcript Content Escaping", () => {
	...
	describe("Real-world use cases", () => {
		...
		it("should handle file path with backslashes", () => {
			...
			expect(result).toBe(
				"Error in file: C:\\\\Program Files\\\\App\\\\config.json",
			);
		});
	...
	describe("JSON serialization", () => {
		...
		it("should validate JSON round-trip integrity", () => {
			...
			expect(parsed.transcript[0].content).toBe(
				"Test\\nwith\\nmultiple\\nlines",
			);
		...
		it("should create valid JSON for webhook payload structure", () => {
			...
			expect(parsed.transcript_ids.transcripts[0].content).toBe(
				"Line 1\\nLine 2",
			);
			expect(parsed.transcript_ids.transcripts[1].content).toBe(
				"Response\\twith\\ttabs",
			);
		});
	});
