...
function generateMockResponse(
	...
	if (content.startsWith("test-error")) {
		...
		responses.push({
			...
			user_id: messagePayload.user_id,
			response:
				"Activity creation failed because an activity with the same name already exists in this tenant.",
			response_group_id: responseGroupId,
			metadata: {
				turn_complete: true,
				completion: {
			...
			if (responses && responses.length > 0) {
				...
				const sendResponsesSequentially = async () => {
					for (let i = 0; i < responses.length; i++) {
						...
						const isReasoning =
							response.metadata?.type === "reasoning" ||
							(response.metadata?.reasoning && !response.response);
						const delay =
							i === 0 ? MOCK_CONFIG.responseDelay : isReasoning ? 1500 : 800;
